...
// MARK: - SaveError (Pretty Core Data diagnostics)
private enum SaveError: Error, LocalizedError {
    ...
    var errorDescription: String? {
        switch self {
        case .coreData(let e):
            return SaveError.describe(e)
        }
    }

    /// Converts to a public-facing Foundation.Error that still carries a readable description.
    func asPublicError() -> Error {
        self
    }

    // MARK: Pretty Printer
    private static func describe(_ error: NSError) -> String {
        // Multi-error container (code 1560) — pick the first with details, but include the count.
        if error.code == NSValidationMultipleErrorsError,
           let nested = (error.userInfo[NSDetailedErrorsKey] as? [NSError]), !nested.isEmpty {
            return "Validation failed (\(nested.count) issues). First: " + describe(nested[0])
        }

        // Pull typical validation keys if present.
        // FIX: Correct Core Data userInfo keys
        ...
        // Common Core Data codes → friendly messages.
        switch error.code {
        ...
        default:
            // Fall back to the system’s description if present.
            if let desc = error.localizedFailureReason, !desc.isEmpty {
                return desc
            }
            return error.localizedDescription // includes domain/code
        }
    }
}
