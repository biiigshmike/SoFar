...
final class PresetsViewModel: ObservableObject {
    ...
    // MARK: API
    /// Fetches global templates and aggregates assignment counts.
    func loadTemplates(using context: NSManagedObjectContext) {
        ...
        for t in templates {
            ...
            // Use the template's own actual amount so the preset value remains
            // unchanged as children are added. This allows the default actual
            // to reflect only manual edits to the template.
