...
enum AppTheme: String, CaseIterable, Identifiable, Codable {
    ...
    /// Shared fill color used by grouped form rows.
    /// - Parameter colorScheme: Active color scheme so light and dark appearances can tune contrast.
    func formRowFill(for colorScheme: ColorScheme) -> Color {
        switch self {
        case .system:
        ...
        default:
            ...
            switch colorScheme {
            case .dark:
                accentTone = AppThemeColorUtilities.adjust(
                    resolvedTint,
                    saturationMultiplier: 0.60,
                    brightnessMultiplier: 0.44,
                    alpha: 1.0
                )
                blendAmount = 0.28
                adjusted = AppThemeColorUtilities.adjust(
                    AppThemeColorUtilities.mix(background, accentTone, amount: blendAmount),
    ...
    /// Stroke color used by grouped form rows.
    /// - Parameter colorScheme: Active color scheme so light and dark appearances can tune contrast.
    func formRowStroke(for colorScheme: ColorScheme) -> Color {
        switch self {
        case .system:
            return Color(UIColor { trait in
                trait.userInterfaceStyle == .dark
        ...
        default:
            let fillColor = formRowFill(for: colorScheme)
            let accentTone: Color
            ...
            case .dark:
                ...
                mixAmount = 0.42
                adjusted = AppThemeColorUtilities.adjust(
                    AppThemeColorUtilities.mix(fillColor, accentTone, amount: mixAmount),
